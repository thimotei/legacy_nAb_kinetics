...
parameters {
  ...
  vector<lower=0>[P] beta_t0; // Coefficients for covariates corresponding to t0
  vector<upper=0>[P] beta_tp; // Coefficients for covariates corresponding to tp
  vector[P] beta_ts; // Coefficients for covariates corresponding to ts
  vector<lower=0>[P] beta_m1; // Coefficients for covariates corresponding to m1
...
model {
  ...
  // Censoring at log2(titre/5) = 7, originally titre = 5120
  ...
  sigma_t0_ind ~ std_normal();
  sigma_tp_ind ~ std_normal();
  sigma_ts_ind ~ std_normal();
  sigma_m1_ind ~ std_normal();
  sigma_m2_ind ~ std_normal();
  sigma_m3_ind ~ std_normal();
