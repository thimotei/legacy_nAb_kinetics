preprocess_data <- function(
  ...
  dt_proc <- dt_copy[, .(id = factor(as.numeric(elig_study_id)),
    ...
    dt_proc <- dt_copy[, .(id = factor(as.numeric(elig_study_id)),
                           ...
                           dose_4_type = factor(dose_4),
                           dose_5_type = factor(dose_5),
                           Ancestral = ic50_wildtype_ds10,
                           Alpha = ic50_Alpha_ds10,
                           Beta = ic50_Beta_ds10,
                           Delta = ic50_Delta_ds10,
                           BA.1 = ic50_Omicron_BA1_ds10,
                           BA.2 = ic50_Omicron_BA2_ds10,
                           BA.5 = ic50_Omicron_BA5_ds10,
                           BQ.1.1 = ic50_BQ.1.1_ds10,
                           XBB = ic50_XBB_ds10,
                           inf_date = lubridate::ymd(episode_start), 
                           inf_num = as.numeric(episode_number),
                           voc = factor(episode_variant_summarised),
    ...
    dt_proc <- dt_copy[, .(id = factor(as.numeric(elig_study_id)),
                           ...
                           # XBB = ic50_XBB,
                           XBB = ic50_XBB,
                           Ancestral_ext = ic50_wildtype_ds10,
                           Alpha_ext = ic50_Alpha_ds10,
                           Delta_ext = ic50_Delta_ds10,
                           BA.1_ext = ic50_Omicron_BA1_ds10,
                           BA.2_ext = ic50_Omicron_BA2_ds10,
                           BA.5_ext = ic50_Omicron_BA5_ds10,
                           BQ.1.1_ext = ic50_BQ.1.1_ds10,
                           XBB_ext = ic50_XBB.1.5_ds10,
                           inf_date = lubridate::ymd(episode_start), 
                           inf_num = as.numeric(episode_number),
                           voc = factor(episode_variant_summarised),
                           vax_num = as.numeric(dose))][order(id, date)]
  ...
  #--- changing IDs (keeping the old ones for recovery later)
  dt_proc[, data_id := id]
  dt_new_ids <- dt_proc[, .(id = .GRP), by = data_id]
  dt_proc[, id := NULL]
  
  dt_proc <- merge(dt_proc, dt_new_ids, by = "data_id")
